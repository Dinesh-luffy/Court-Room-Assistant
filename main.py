...
from rag import load_pdf_text, load_json_qa, chunk_text, store_in_faiss, retrieve_context
...
def list_cases():
    ...
    if not os.path.exists(BASE_DB_PATH):
        ...
        return []
    cases = [d for d in os.listdir(BASE_DB_PATH) if os.path.isdir(os.path.join(BASE_DB_PATH, d)) and d != 'core_knowledge']
    if not cases:
        print("⚠️ No cases found.")
    else:
        print("\n📂 Available Cases:")
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not current_case_db_path:
                ...
                continue
            path_input = input("Enter full path to PDF or 'browse' for file dialog: ").strip()
            if path_input.lower() == "voice":
                pdf_path = get_voice_input()
            elif path_input.lower() == "browse":
                pdf_path = browse_file(file_types=[("PDF files", "*.pdf")])
            else:
                pdf_path = path_input
            ...
            query = get_voice_input() if query_input.lower() == "voice" else query_input
            if query:
            ...
            if query:
                context = ""
                if os.path.exists(os.path.join(CORE_DB_PATH, "index.faiss")):
                    context = retrieve_context(query, CORE_DB_PATH, top_k=5)

                if not context or len(context.strip()) < 50:
                    print("⚠️ No strong match in core knowledge base, falling back to model knowledge.")
                    answer = generate_answer(query, context="")
                else:
                    answer = generate_answer(query, context=context)
            ...
            opponent_text = get_voice_input() if opponent_input.lower() == "voice" else opponent_input
            if opponent_text:
                context = retrieve_context(opponent_text, current_case_db_path, top_k=3)
                query = f"The opponent argued: {opponent_text}"
                answer = generate_answer(query, context)
